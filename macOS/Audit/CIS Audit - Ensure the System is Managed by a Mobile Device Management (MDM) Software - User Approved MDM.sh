...
#
#   BENCHMARK NAME:
#   CIS Audit - Ensure the System is Managed by a Mobile Device Management (MDM) Software - User Approved MDM
#
...
audit(){
    if (( "${os_version}" > "${maximum_macOS_version_required}" )); then
        ...
        audit_MDM_enrollment="$(sudo /usr/bin/profiles status -type enrollment | /usr/bin/awk -F: '/MDM enrollment/ {print $2}' | /usr/bin/grep -c "Yes (User Approved)")"

        if [ "${audit_MDM_enrollment}" = "1" ]; then
            audit_MDM_enrollment="true"
        else
            audit_MDM_enrollment="false"
        fi

        echo "<result>${audit_MDM_enrollment}</result>"
    else
