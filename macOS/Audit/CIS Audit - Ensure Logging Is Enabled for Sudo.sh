...
#
#   BENCHMARK NAME:
#   CIS Audit - Ensure Logging Is Enabled for Sudo
#
...
#
#   AUDIT: 
#   Verify that the default sudoers controls are in place for logging of sudo commands.
#
####################################################################################################
#
#   CHANGE CONTROL LOG
...
audit(){
    if (( "${os_version}" > "${maximum_macOS_version_required}" )); then
        ...
        audit_logging_for_sudo="$(/usr/bin/sudo /usr/bin/sudo -V | /usr/bin/grep -c "Log when a command is allowed by sudoers")"

        if [ "${audit_logging_for_sudo}" = "1" ]; then
