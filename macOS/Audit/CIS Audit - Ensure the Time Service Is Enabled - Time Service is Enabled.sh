...
#
#   BENCHMARK NAME:
#   CIS Audit - Ensure the Time Service Is Enabled - Time Service is Enabled
#
...
#
#   AUDIT: 
#   Verify that the 'timed' service is enabled.
#
####################################################################################################
#
#   CHANGE CONTROL LOG
...
audit(){
    if (( "${os_version}" > "${maximum_macOS_version_required}" )); then
        ...
        audit_timed_service="$(/usr/bin/sudo /bin/launchctl list | /usr/bin/grep -c com.apple.timed)"

        if [ ${audit_timed_service} = "1" ]; then
