...
#
#   BENCHMARK NAME:
#   CIS Audit - Ensure the root Account Is Disabled - Root User Is Disabled
#
...
#
#   AUDIT: 
#   Verify the the root user is disabled.
#
####################################################################################################
#
#   CHANGE CONTROL LOG
...
audit(){
    if (( "${os_version}" > "${maximum_macOS_version_required}" )); then
        ...
        audit_root_user_status="$(/usr/bin/sudo /usr/bin/dscl . -read /Users/root AuthenticationAuthority 2>&1)"

        if [[ ${audit_root_user_status} == *"No such key"* ]]; then
