...
#
#   BENCHMARK NAME:
#   CIS Audit - Ensure the Time Service Is Enabled - Time Offset
#
#   PROFILE APPLICABILITY: 
#   Level 1
#
#   DESCRIPTION: 
#   In macOS 10.14, Apple replaced ntp with timed for time services, and is used to ensure
...
audit(){
    if (( "${os_version}" > "${maximum_macOS_version_required}" )); then
        ...
        audit_offset_in_seconds="$(/usr/bin/sudo /usr/bin/sntp ${audit_getnetworktimeserver} | /usr/bin/awk '{print $1}' | /usr/bin/cut -c2-)"

        echo "<result>${audit_offset_in_seconds} seconds</result>"
    else
