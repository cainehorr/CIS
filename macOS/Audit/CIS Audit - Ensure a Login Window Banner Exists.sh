...
#
#   BENCHMARK NAME:
#   CIS Audit - Ensure a Login Window Banner Exists
#
...
audit(){
    if (( "${os_version}" > "${maximum_macOS_version_required}" )); then
        ...
        # audit_PolicyBanner="$(/usr/bin/sudo /bin/cat /Library/Security/PolicyBanner.*)"

        if [ -f  "/Library/Security/PolicyBanner.txt" ] || [ -f  "/Library/Security/PolicyBanner.rtf" ]; then
