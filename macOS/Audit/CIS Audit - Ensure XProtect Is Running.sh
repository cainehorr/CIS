...
#
#   BENCHMARK NAME:
#   CIS Audit - Ensure XProtect Is Running
#
...
#
#   AUDIT: 
#   Verify that XProtect is running.
#
####################################################################################################
#
#   CHANGE CONTROL LOG
...
audit(){
    if (( "${os_version}" > "${maximum_macOS_version_required}" )); then
        ...
        audit_xprotect_running_status="$(/usr/bin/sudo /bin/launchctl list | /usr/bin/grep -cE "(com.apple.XprotectFramework.PluginService$|com.apple.XProtect.daemon.scan$)")"

        if [ "${audit_xprotect_running_status}" = "2" ]; then
