...
#
#   BENCHMARK NAME:
#   CIS Audit - Ensure Sleep and Display Sleep Is Enabled on Apple Silicon Devices - Sleep Enabled
#
...
#
#   AUDIT: 
#   Verify sleep settings are enabled.
#
####################################################################################################
#
#   CHANGE CONTROL LOG
...
audit(){
    if (( "${os_version}" > "${maximum_macOS_version_required}" )); then
        ...
        audit_sleep_settings="$(/usr/bin/sudo /usr/bin/pmset -b -g | /usr/bin/grep -e "^ sleep" | /usr/bin/awk '{print $2}')"

        if [ "${audit_sleep_settings}" -le "15" ]; then
