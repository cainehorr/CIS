...
#
#   BENCHMARK NAME:
#   CIS Audit - Ensure System Integrity Protection Status (SIP) Is Enabled
#
...
#
#   AUDIT: 
#   Verify that System Integrity Protection is enabled.
#
####################################################################################################
#
#   CHANGE CONTROL LOG
...
audit(){
    if (( "${os_version}" > "${maximum_macOS_version_required}" )); then
        ...
        audit_SIP_status="$(/usr/bin/sudo /usr/bin/csrutil status)"

        if [ "${audit_SIP_status}" = "System Integrity Protection status: enabled." ]; then
