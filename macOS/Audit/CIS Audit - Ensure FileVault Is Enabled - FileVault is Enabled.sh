...
#
#   BENCHMARK NAME:
#   CIS Audit - Ensure FileVault Is Enabled - FileVault is Enabled
#
...
audit(){
    if (( "${os_version}" > "${maximum_macOS_version_required}" )); then
        ...
        audit_fdesetup_status="$(/usr/bin/sudo /usr/bin/fdesetup status)"

        if [ "${audit_fdesetup_status}" = "FileVault is On." ]; then
