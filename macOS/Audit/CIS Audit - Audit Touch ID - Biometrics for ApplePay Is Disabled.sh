...
#
#   BENCHMARK NAME:
#   CIS Audit - Audit Touch ID - Biometrics for ApplePay Is Disabled
#
...
#
#   AUDIT: 
#   Verify Touch ID settings for ApplePay.
#
####################################################################################################
#
#   CHANGE CONTROL LOG
...
audit(){
    if (( "${os_version}" > "${maximum_macOS_version_required}" )); then
        ...
        audit_biometrics_for_applepay="$(/usr/bin/sudo -u ${currentUser} /usr/bin/bioutil -r | /usr/bin/awk -F: '/Biometrics for ApplePay:/ {print $2}' | /usr/bin/sed 's/^[[:space:]]*//g')"

        if [ "${audit_biometrics_for_applepay}" = "0" ]; then
