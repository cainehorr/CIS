...
#
#   BENCHMARK NAME:
#   CIS Audit - Ensure Warn When Visiting A Fraudulent Website in Safari Is Enabled - Profile Installed
#
...
#
#   AUDIT: 
#   Verify that warn when visiting a fraudulent site in Safari is enabled.
#
####################################################################################################
#
#   CHANGE CONTROL LOG
...
audit(){
    if (( "${os_version}" > "${maximum_macOS_version_required}" )); then
        ...
        audit_WarnAboutFraudulentWebsites="$(/usr/bin/sudo /usr/sbin/system_profiler SPConfigurationProfileDataType | /usr/bin/grep WarnAboutFraudulentWebsites | /usr/bin/tr -d ' ')"

        if [ "${audit_WarnAboutFraudulentWebsites}" = "WarnAboutFraudulentWebsites = 1;" ]; then
