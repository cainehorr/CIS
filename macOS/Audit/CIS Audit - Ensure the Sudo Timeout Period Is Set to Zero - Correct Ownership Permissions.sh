...
#
#   BENCHMARK NAME:
#   CIS Audit - Ensure the Sudo Timeout Period Is Set to Zero - Correct Ownership Permissions
#
...
audit(){
    if (( "${os_version}" > "${maximum_macOS_version_required}" )); then
        ...
        audit_sudoers_owner="$(/usr/bin/stat /etc/sudoers.d | /usr/bin/awk '{print $5}')"
        audit_sudoers_group="$(/usr/bin/stat /etc/sudoers.d | /usr/bin/awk '{print $6}')"

        if [ "${audit_sudoers_owner}" = "root" ] && [ "${audit_sudoers_group}" = "wheel" ]; then
